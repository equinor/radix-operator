...
func (s *syncerTestSuite) Test_ReconcileStatus() {
	...
	sut := s.createSyncer(rb, config.Config{})
...
func (s *syncerTestSuite) Test_ShouldSkipReconcileResourcesWhenBatchConditionIsDone() {
	...
	for i, conditionType := range doneConditions {
		s.Run(string(conditionType), func() {
			...
			s.Require().NoError(err)
			sut := s.createSyncer(batch, config.Config{})
...
func (s *syncerTestSuite) Test_Batch_ImagePullSecrets() {
	...
	for testName, test := range tests {
		s.Run(testName, func() {
			...
			cfg := config.Config{Operator: config.OperatorConfig{ExternalRegistryAuthSecret: test.defaultRegistryAuthSecret}}
...
func (s *syncerTestSuite) Test_BatchJobStatus() {
	...
	for testName, test := range tests {
		s.Run(testName, func() {
			...
			// Run test
			sut := s.createSyncer(batch, config.Config{}, WithClock(clock.NewFakeClock(now)))
			s.Require().NoError(sut.OnSync(context.Background()))
