...
func (kubeutil *Kube) DeleteServiceAccount(ctx context.Context, namespace, name string) error {
	...
	if err != nil && errors.IsNotFound(err) {
		...
		return fmt.Errorf("failed to get service account object: %w", err)
	}
	if err = kubeutil.kubeClient.CoreV1().ServiceAccounts(namespace).Delete(ctx, name, metav1.DeleteOptions{}); err != nil {
		return fmt.Errorf("failed to delete ServiceAccount object: %w", err)
	}
	return nil
}
...
func (kubeutil *Kube) ListServiceAccountsWithSelector(ctx context.Context, namespace string, labelSelectorString string) ([]*corev1.ServiceAccount, error) {
	...
	if kubeutil.ServiceAccountLister != nil {
		...
		serviceAccounts = slice.PointersOf(list.Items)
	}
