...
func Test_CreateRadixApplication_LimitMemoryIsTakenFromRequestsMemory(t *testing.T) {
	...
	assert.Equal(t, "200Mi", ra.Spec.Components[2].Resources.Limits["memory"], "server3 not expected resource limits memory")
	assert.Equal(t, "200Mi", ra.Spec.Components[3].Resources.Limits["memory"], "server4 not expected resource limits memory")
