...
func TestGetRadixComponentsForEnv_PublicPort_OldPublic(t *testing.T) {
	...
	deployComponent, _ := GetRadixComponentsForEnv(ra, nil, env, componentImages, envVarsMap, nil)
...
func TestGetRadixComponentsForEnv_ListOfExternalAliasesForComponent_GetListOfAliases(t *testing.T) {
	...
	deployComponent, _ := GetRadixComponentsForEnv(ra, nil, "prod", componentImages, envVarsMap, nil)
	...
	deployComponent, _ = GetRadixComponentsForEnv(ra, nil, "dev", componentImages, envVarsMap, nil)
	assert.Equal(t, 2, len(deployComponent))
...
func TestGetRadixComponentsForEnv_CommonEnvironmentVariables_No_Override(t *testing.T) {
	...
	deployComponentProd, _ := GetRadixComponentsForEnv(ra, nil, "prod", componentImages, envVarsMap, nil)
	...
	deployComponentDev, _ := GetRadixComponentsForEnv(ra, nil, "dev", componentImages, envVarsMap, nil)
...
func TestGetRadixComponentsForEnv_Monitoring(t *testing.T) {
	...
	comps, err := GetRadixComponentsForEnv(radApp, nil, envs[0], componentImages, envVarsMap, nil)
	assert.Nil(t, err)
	...
	comps, err = GetRadixComponentsForEnv(radApp, nil, envs[1], componentImages, envVarsMap, nil)
	assert.Nil(t, err)
...
func Test_GetRadixComponents_NodeName(t *testing.T) {
	...
	t.Run("override job gpu and gpu-count with environment gpu and gpu-count", func(t *testing.T) {
		t.Parallel()
		deployComponent, _ := GetRadixComponentsForEnv(ra, nil, "env1", componentImages, envVarsMap, nil)
	...
	t.Run("override job gpu-count with environment gpu-count", func(t *testing.T) {
		t.Parallel()
		deployComponent, _ := GetRadixComponentsForEnv(ra, nil, "env2", componentImages, envVarsMap, nil)
	...
	t.Run("override job gpu with environment gpu", func(t *testing.T) {
		t.Parallel()
		deployComponent, _ := GetRadixComponentsForEnv(ra, nil, "env3", componentImages, envVarsMap, nil)
	...
	t.Run("do not override job gpu or gpu-count with environment gpu or gpu-count", func(t *testing.T) {
		t.Parallel()
		deployComponent, _ := GetRadixComponentsForEnv(ra, nil, "env4", componentImages, envVarsMap, nil)
		assert.Equal(t, compGpu, deployComponent[0].Node.Gpu)
		assert.Equal(t, compGpuCount, deployComponent[0].Node.GpuCount)
	})
}
...
func Test_GetRadixComponentsForEnv_Identity(t *testing.T) {
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			...
			components, err := GetRadixComponentsForEnv(ra, nil, envName, make(pipeline.DeployComponentImages), make(radixv1.EnvVarsMap), nil)
			require.NoError(t, err)
...
func TestGetRadixComponentsForEnv_ImageWithImageTagName(t *testing.T) {
	...
	for _, ts := range scenarios {
		t.Run(ts.name, func(t *testing.T) {
			...
			deployComponents, err := GetRadixComponentsForEnv(ra, nil, environment, componentImages, make(radixv1.EnvVarsMap), nil)
...
func Test_GetRadixComponents_CustomHealthChecks(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.description, func(t *testing.T) {
			...
			deployComponents, err := GetRadixComponentsForEnv(ra, nil, "dev", make(pipeline.DeployComponentImages), make(radixv1.EnvVarsMap), nil)
			require.NoError(t, err)
...
func Test_GetRadixComponents_ReplicasOverride(t *testing.T) {
	...
	for description, testCase := range testCases {
		t.Run(description, func(t *testing.T) {
			...
			deployComponents, err := GetRadixComponentsForEnv(ra, activeRd, env, componentImages, envVarsMap, nil)
			require.NoError(t, err)
			require.Len(t, deployComponents, 1)
...
func Test_GetRadixComponents_HorizontalScaling(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.description, func(t *testing.T) {
			...
			deployComponents, _ := GetRadixComponentsForEnv(ra, nil, env, componentImages, envVarsMap, nil)
	...
	for _, testCase := range testCases {
		t.Run(testCase.description, func(t *testing.T) {
			...
			for _, envName := range []string{env1, env2} {
				deployComponents, _ := GetRadixComponentsForEnv(ra, nil, envName, componentImages, envVarsMap, nil)
				deployComponent, exists := slice.FindFirst(deployComponents, func(component radixv1.RadixDeployComponent) bool {
					return component.Name == componentName
				})
				require.True(t, exists)
...
func Test_GetRadixComponents_VolumeMounts(t *testing.T) {
	...
	for testName, testCase := range testCases {
		t.Run(testName, func(t *testing.T) {
			...
			deployComponents, _ := GetRadixComponentsForEnv(ra, nil, env, nil, nil, nil)
			deployComponent, exists := slice.FindFirst(deployComponents, func(component radixv1.RadixDeployComponent) bool {
				return component.Name == componentName
			})
			require.True(t, exists)
...
func Test_GetRadixComponentsForEnv_Runtime_AlwaysUseFromDeployComponentImages(t *testing.T) {
	...
	for name, test := range tests {
		t.Run(name, func(t *testing.T) {
			deployComponents, err := GetRadixComponentsForEnv(ra, nil, test.env, test.deployImages, make(radixv1.EnvVarsMap), nil)
			require.NoError(t, err)
			require.Len(t, deployComponents, 1)
