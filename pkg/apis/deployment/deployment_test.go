...
func setupTest(t *testing.T) (*test.Utils, kubernetes.Interface, *kube.Kube, radixclient.Interface, prometheusclient.Interface, secretProvider.Interface) {
	// Setup
...
func Test_JobScheduler_ObjectsGarbageCollected(t *testing.T) {
	...
	testTheory := func(theory *theoryData) {
		...
		t.Run(theory.name, func(t *testing.T) {
			...
			_, err := applyDeploymentWithSync(tu, client, kubeUtil, radixclient, prometheusclient, theory.builder)
			require.NoError(t, err, "error apply deployment")
...
func Test_IngressAnnotations_Called(t *testing.T) {
	...
	for _, ing := range ingresses.Items {
		assert.Equal(t, expected, ing.GetAnnotations())
	}
}
