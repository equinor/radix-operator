	...
	for _, componentsExist := range []bool{true, false} {
		...
		t.Run("Test Suite", func(t *testing.T) {
			...
			if componentsExist {
				...
				_, err := applyDeploymentWithSync(tu, kubeclient, kubeUtil, radixclient, existingRadixDeploymentBuilder)
			...
			t.Run(fmt.Sprintf("%s: validate secrets", testScenario), func(t *testing.T) {
				...
				if !componentsExist {
					assert.True(t, secretByNameExists(fmt.Sprintf("%s-%s-blobfusecreds", componentNameRadixQuote, "blob"), secrets), "Volume mount secret")
				} else {
					assert.False(t, secretByNameExists(fmt.Sprintf("%s-blobfusecreds", componentNameRadixQuote), secrets), "TLS certificate for external alias is not properly defined")
				}
			})
