...
func (s *OAuthProxyResourceManagerTestSuite) Test_Sync_OAuthProxyIngressesCreated() {
	...
	s.ingressAnnotationProvider.EXPECT().GetAnnotations(expectedIngServerCall, "test-namespace").Times(1).Return(expectedIngServerAnnotations, nil)
	...
	s.ingressAnnotationProvider.EXPECT().GetAnnotations(expectedIngWebCall, "test-namespace").Times(2).Return(expectedIngWebAnnotations, nil)
