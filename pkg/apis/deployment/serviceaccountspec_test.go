...
import (
	deployment2 "github.com/equinor/radix-operator/pkg/apis/internal/deployment"
	"testing"
...
func Test_ServiceAccountSpec(t *testing.T) {
	...
	t.Run("app", func(t *testing.T) {
		...
		spec = NewServiceAccountSpec(rd, deployment2.newJobSchedulerComponent(&rd.Spec.Jobs[0], rd))
		...
		spec = NewServiceAccountSpec(rd, deployment2.newJobSchedulerComponent(&rd.Spec.Jobs[1], rd))
		assert.Equal(t, pointers.Ptr(true), spec.AutomountServiceAccountToken())
		assert.Equal(t, defaults.RadixJobSchedulerServiceName, spec.ServiceAccountName())
