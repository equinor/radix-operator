...
# It serves repositories over SSH and accepts any client public key (see gitserver/any-key.sh),
...
COPY gitserver/sshd_config /etc/gitserver/sshd_config
COPY gitserver/any-key.sh /usr/local/bin/any-key
COPY gitserver/entrypoint.sh /usr/local/bin/entrypoint.sh
