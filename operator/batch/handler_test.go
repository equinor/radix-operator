...
func (s *handlerTestSuite) Test_RadixScheduleJobNotFound() {
	sut := NewHandler(s.kubeClient, s.kubeUtil, s.radixClient, s.eventRecorder, config2.Config{}, WithSyncerFactory(s.syncerFactory))
...
func (s *handlerTestSuite) Test_RadixScheduledExist_SyncerError() {
	...
	sut := NewHandler(s.kubeClient, s.kubeUtil, s.radixClient, s.eventRecorder, config2.Config{}, WithSyncerFactory(s.syncerFactory))
	s.syncerFactory.EXPECT().CreateSyncer(s.kubeClient, s.kubeUtil, s.radixClient, rr, job, config2.Config{}).Return(s.syncer).Times(1)
