...
func (s *controllerTestSuite) Test_RadixAlertEvents() {
	...
	s.Handler.EXPECT().Sync(gomock.Any(), namespace, alertName).DoAndReturn(s.SyncedChannelCallback()).Times(1)
	...
	s.Handler.EXPECT().Sync(gomock.Any(), namespace, alertName).DoAndReturn(s.SyncedChannelCallback()).Times(0)
	_, err = s.RadixClient.RadixV1().RadixAlerts(namespace).Update(context.Background(), alert, metav1.UpdateOptions{})
	s.Require().NoError(err)
...
func (s *controllerTestSuite) Test_RadixRegistrationEvents() {
	...
	s.Handler.EXPECT().Sync(gomock.Any(), namespace, alert1Name).DoAndReturn(s.SyncedChannelCallback()).Times(0)
