...
import (
	...
	"github.com/equinor/radix-operator/operator/alert"
	...
	"github.com/equinor/radix-operator/operator/job"
...
func (a *App) createSchedulers(ctx context.Context) ([]scheduler.TaskScheduler, error) {
	...
	if envCleanupTask, err := scheduler.NewTaskScheduler(ctx,
		tasks.NewRadixEnvironmentsCleanup(ctx, a.kubeUtil, a.cfg.Operator.OrphanedRadixEnvironmentsRetentionPeriod),
