...
func (s *applyConfigTestSuite) Test_BuildKitOptions() {
	const appName = "anyapp"
	scenarios := map[string]struct {
		...
		//TODO continue
		"useBuildKit=nil, useBuildCache=nil, overrideUseBuildCache=nil, refreshBuildCache=nil": {useBuildKit: nil, useBuildCache: nil, overrideUseBuildCache: nil, refreshBuildCache: nil},
	}
	for name, ts := range scenarios {
		s.T().Run(name, func(t *testing.T) {
			...
			expectedRa := utils.ARadixApplication().WithAppName(appName).WithBuildKit(ts.useBuildKit).WithBuildCache(ts.useBuildCache).BuildRA()
			pipelineInfo := &model.PipelineInfo{
				PipelineArguments: model.PipelineArguments{
					AppName:               appName,
					PipelineType:          "build-deploy",
					Branch:                "master",
					OverrideUseBuildCache: ts.overrideUseBuildCache,
