...
func (step *RunPipelinesStepImplementation) getPipelineParams(pipeline *pipelinev1.Pipeline, targetEnv string, pipelineInfo *model.PipelineInfo) ([]pipelinev1.Param, error) {
	...
	subPipelineParams, ok := pipelineInfo.EnvironmentSubPipelineParams[targetEnv]
	if !ok {
		return nil, fmt.Errorf("missing sub-pipeline params for target environment %q", targetEnv)
	}
