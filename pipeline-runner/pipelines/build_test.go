...
func Test_getGitCloneCommand(t *testing.T) {
	gitCloneCommand := getGitCloneCommand("git@github.com:Statoil/radix-github-webhook.git", "master")

	assert.Equal(t, "git clone git@github.com:Statoil/radix-github-webhook.git -b master .", gitCloneCommand)
}
...
func Test_getInitContainerArgString_NoCommitID(t *testing.T) {
	...
	assert.Equal(t, "apk add --no-cache bash openssh-client git && ls /root/.ssh && cd /workspace && git clone git@github.com:Statoil/radix-github-webhook.git -b master .", argString)
}
...
func Test_getInitContainerArgString_WithCommitID(t *testing.T) {
	...
	assert.Equal(t, "apk add --no-cache bash openssh-client git && ls /root/.ssh && cd /workspace && git clone git@github.com:Statoil/radix-github-webhook.git -b master . && git checkout 762235917d04dd541715df25a428b5f62394834c", argString)
}
