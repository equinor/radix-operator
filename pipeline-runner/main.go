...
func setPipelineArgsFromArguments(cmd *cobra.Command, pipelineArgs *model.PipelineArguments, arguments []string) error {
	...
	cmd.Flags().StringVar(&pipelineArgs.DeploymentName, flags.PromoteDeploymentName, "", "Radix deployment name to promote")
	cmd.Flags().StringVar(&pipelineArgs.FromEnvironment, flags.PromoteFromEnvironment, "", "Radix application environment name to promote from")
	...
	cmd.Flags().StringVar(&pipelineArgs.RadixConfigFile, flags.RadixConfigFileName, "", "Radix config file name. Example: radixconfig.yaml")
	...
	cmd.Flags().StringVar(&pipelineArgs.GitWorkspace, flags.GithubWorkspace, git.Workspace, fmt.Sprintf("(Optional) Workspace path to the cloned GitHub repository. Default %s", git.Workspace))
