...
func setPipelineArgsFromArguments(cmd *cobra.Command, pipelineArgs *model.PipelineArguments, arguments []string) error {
	...
	cmd.Flags().BoolVar(&pipelineArgs.TriggeredFromWebhook, defaults.RadixPipelineTriggeredFromWebhook, false, fmt.Sprintf("(Optional) The pipeline job is triggered from the GiHub webhook. Default false"))
