...
// RestoreSecretPlugin is a restore item action plugin for Velero
type RestoreSecretPlugin struct {
...
func (p *RestoreSecretPlugin) AppliesTo() (velero.ResourceSelector, error) {
...
func (p *RestoreSecretPlugin) Execute(input *velero.RestoreItemActionExecuteInput) (*velero.RestoreItemActionExecuteOutput, error) {
	var secret corev1.Secret
	if err := runtime.DefaultUnstructuredConverter.FromUnstructured(input.ItemFromBackup.UnstructuredContent(), &secret); err != nil {
		return nil, fmt.Errorf("unable to convert unstructured item to Secret: %w", err)
	}

	logger := getLoggerForObject(p.Log, &secret)
